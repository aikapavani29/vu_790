...
CREATE TABLE finance_fees ( fee_id INT PRIMARY KEY, student_id INT,amount DECIMAL(10, 2), payment_date DATE);
...
SELECT s.student_id,s.first_name,SUM(f.amount) AS total_fees 
FROM students s 
JOIN finance_fees f ON s.student_id=f.student_id 
GROUP BY s.student_id,s.first_name;

SELECT c.course_name,AVG(e.grade) AS avearage_grade 
FROM courses c 
...
SELECT MAX(amount)highest_fees
FROM finance_fees;

SELECT MIN(amount)lowest_fees
FROM finance_fees;

SELECT SUM(f.amount)AS total_fees_2023
FROM finance_fees f
WHERE YEAR(f.payment_date)=2023;

SELECT MAX(e.grade) AS second_highest_grade
FROM enrollments e
JOIN courses c ON e.course_id=c.course_id
WHERE c.course_name='Data structures'
AND e.grade<(SELECT MAX(e.grade)
FROM enrollments e
JOIN courses c ON e.course_id=c.course_id
WHERE c.course_name='Data structures');
